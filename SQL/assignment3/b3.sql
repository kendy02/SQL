...
-- 3. Lấy thông tin account có full name dài nhất dùng CHAR_LENGTH: dùng cho dấu
SELECT full_name,CHAR_LENGTH(full_name)
FROM account
ORDER BY CHAR_LENGTH(full_name) DESC
LIMIT 1;
-- 4. Lấy thông tin account có full_name dài nhất và thuộc phòng ban có id =3
SELECT *
FROM account
WHERE department_id =3
ORDER BY CHAR_LENGTH(full_name) DESC
LIMIT 1;
-- 5. Lấy ra tên group đã tham gia trước ngày 20/12/2019
SELECT group_name, create_date
FROM  `group`
WHERE create_date < "2019-12-20";
-- 6. Lấy ra id của question có >=4 câu trả lời 
SELECT question_id
FROM answer
GROUP BY question_id
HAVING COUNT(answer_id) >=4;
-- 7. Lấy ra các mã đề thi có thời gian thi >=60 phút và được tạo trước ngày 20/12/2019 
SELECT code
FROM exam
WHERE duration >= 60 AND create_date < "2019-12-20";
-- 8. Lấy ra 5 group được tạo gần đây nhất 
SELECT *
FROM `group`
ORDER BY create_date DESC
LIMIT 5;

-- 9. Đểm số nhân viên thuộc department có id=2 
SELECT COUNT(account_id)
FROM account
GROUP BY department_id
HAVING department_id = 2;

-- 10. Lấy ra nhân viên có tên bắt đầu bằng "D" và kết thúc bằng "o"
SELECT full_name
FROM account
...
-- 12. update thông tin của account có id = 5 thành tên "Nguyễn Bá Lộc" và email thành loc.nguyenba@cưvti.com.vn
UPDATE account 
SET full_name="Nguyễn Bá Lộc", email="loc.nguyenba@cuvti.com"
WHERE account_id=5;
--  13. update account có id =5 sẽ thuộc group có id = 4
